...
import { fetchSchoolClasses, fetchStudentsByClass } from "@/app/actions";
...
export function useStudents(selectedClassId) {
  ...
  useEffect(() => {
    async function fetchClasses() {
      try {
        setLoading(true);
        const classesData = await fetchSchoolClasses();
  ...
  useEffect(() => {
    async function fetchStudents() {
      try {
        setLoading(true);
        const studentsData = await fetchStudentsByClass(selectedClassId);
